...
# Convert text into embeddings and save FAISS index
def get_vector_store(text_chunks):
    embeddings = HuggingFaceBgeEmbeddings()
    vector_store = FAISS.from_texts(text_chunks, embedding=embeddings)
    vector_store.save_local('faiss_index')
...
def get_conversational_chain():
    prompt_template = """
        You are a research assistant helping analyze academic papers. Use the provided context to answer questions.
...
def process_user_input(user_question, chat_history):
    embeddings = HuggingFaceBgeEmbeddings()
    new_db = FAISS.load_local('faiss_index', embeddings, allow_dangerous_deserialization=True)
    docs = new_db.similarity_search(user_question)
...
def main():
    st.set_page_config("Scholar Bot")
    st.title("📚 Scholar Bot - Research Assistant")
    ...
    for question, answer in st.session_state['chat_history']:
        ...
        with st.chat_message("assistant"):
            st.markdown(f"**Scholar Bot:** {answer}")
    ...
    if user_question:
        ...
        response_text = process_user_input(user_question, st.session_state['chat_history'])
        with st.chat_message("assistant"):
            st.markdown(f"**Scholar Bot:** {response_text}")
